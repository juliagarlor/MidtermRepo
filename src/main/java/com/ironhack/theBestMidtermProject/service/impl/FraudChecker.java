...
//        if the emisor do not have previous transactions, the upper will return null and will give problems, so:
        if (transactionsToday == null){
            transactionsToday = transactions.getAmount().getAmount();
        }else {
...
//        of the previous day, we declare NOT FRAUDULENT:
        BigDecimal topPercentage = new BigDecimal("1.50");
        if (previousTransactions.size() == 0 || transactionsToday.compareTo(max.multiply(topPercentage)) < 0){
            return false;
        }
...
//        Searching for a list of transactions in the last second, good luck
        ...
        System.out.println(transactionsOfLastSecond.size());
